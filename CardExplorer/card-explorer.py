...
#Might have to declare these as local variables later
...
class MyBackground(model.Background):
  ...
  #The Read Card button
    def on_cmdReadCard_mouseClick(self, event):
        cardservice.connection.connect()
        self.components.txtCardATR.text = toHexString( cardservice.connection.getATR() )
        self.components.txtReaderDetails.text = cardservice.connection.getReader()
        self.components.txtCheckSum.text = "0x%X" % atr.getChecksum()
        self.components.txtCheckSumOk.text = atr.checksumOK
